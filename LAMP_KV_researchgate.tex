...
\author{John Cheung}
\affil{College of Professional and Continuing Education}
\date{May 2026}
...
\noindent\textbf{Artifact availability:} The supplementary testing program and README are available at \url{https://github.com/johncheungmk/LAMP-KV}.
...
\begin{enumerate}
    ...
    \item It provides a reproducible Python artifact for peer review at \url{https://github.com/johncheungmk/LAMP-KV}, supporting synthetic KV tensors and optional real KV tensors in NPZ format.
...
\begin{itemize}
    \item \texttt{paper.tex}: LaTeX source;
...
\end{lstlisting}
Use \texttt{--out results/name.json} to save individual results. Use \texttt{scripts/summarize\_results.py} to convert saved JSON files to CSV. The README in the artifact contains exact Windows commands and peer-review instructions. The current project repository is \url{https://github.com/johncheungmk/LAMP-KV}.
...
\section{Conclusion}
LAMP-KV studies a practical policy question for memory-constrained LLM inference: how aggressively should the KV cache be quantized under changing memory pressure? The pilot results show that a simple block-level policy can smoothly transition from conservative INT8 behavior to aggressive INT4 behavior and produce useful intermediate memory/error tradeoffs. The work is not a replacement for established KV quantization methods; rather, it is a policy layer and reproducible artifact for studying adaptive precision selection. The accompanying scripts and README are intended for repository-based peer review and future extension to real KV tensors and end-to-end LLM evaluation. The project repository is available at \url{https://github.com/johncheungmk/LAMP-KV}.
...
\bibitem{cheung2026lampkvrepo}
J. Cheung, ``LAMP-KV supplementary artifact and testing scripts,'' GitHub repository, 2026. Available: \url{https://github.com/johncheungmk/LAMP-KV}.
\end{thebibliography}
